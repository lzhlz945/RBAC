                ...
                <div class="panel-body">
                    <form role="form" id="formBtn1" action="${Path_APP}/user/doAdd" method="post">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="permissionname" value="${permission.name}" name="permissionname" placeholder="请输入许可名称">
                        </div>
                        <div class="form-group">
                            <label for="exampleInputPassword1">权限地址</label>
                            <input type="text" class="form-control" id="url" name="url" value="${permission.url}" placeholder="请输入权限地址">
                        </div>
                        <%--<div class="form-group">--%>
                        ...
                        <%--</div>--%>
                        <button id="editBtn" type="button" class="btn btn-success"><i class="glyphicon glyphicon-plus" ></i> 修改</button>
...
<script type="text/javascript">
    ...
    function toAdd() {
        ...
        $.ajax({
            ...
            success:function (data) {
                ...
                if(data.success){
                    window.location.href="${Path_APP}/permission/index";
                }else {
    ...
    function checkMenuName() {
        var loadingIndex =null;
        $.ajax({
            url:"${Path_APP}/permission/checkPermissionName",
            ...
            success:function (data) {
                layer.close(loadingIndex);
                if(data.success){
                    layer.msg("菜单名称可以使用", {time:2000, icon:1, shift:1}, function () {
                    });
                    $("#updateBtn").attr("disabled",false);

                }else {
                    if(data.data.name == $("#permissionname").val()){
                        layer.msg("菜单名称没有变动", {time:2000, icon:1, shift:1}, function () {
                        });
                        $("#updateBtn").attr("disabled",false);
                    }else {
                        layer.msg("菜单名称已存在", {time:2000, icon:5, shift:6}, function () {
                        });
